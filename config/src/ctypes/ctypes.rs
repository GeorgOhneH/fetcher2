...
impl CTypes {
    pub(crate) fn consume_value(&mut self, mut value: Value) -> Result<(), RequiredError> {
        match self {
            ...
            CTypes::Integer(cinteger) => {
                match value {
                    Value::Number(num) => {
                        match num.as_i64() {
                            Some(int) => {
                                cinteger.set(Some(int as isize));
                                Ok(())
                            },
