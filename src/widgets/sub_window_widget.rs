...
impl<T: Data> Widget<SubWindowInfo<T>> for SubWindow<T> {
    ...
    fn lifecycle(&mut self, ctx: &mut LifeCycleCtx, event: &LifeCycle, data: &SubWindowInfo<T>, env: &Env) {
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &SubWindowInfo<T>, env: &Env) -> Size {
