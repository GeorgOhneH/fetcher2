...
pub enum SplitOrBox<T> {
    Split(Box<Split<T>>),
    Box(Box<dyn Widget<T>>),
}

impl<T: Data> Widget<T> for SplitOrBox<T>  {
    fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut T, env: &Env) {
        match self {
            Self::Box(widget) => widget.event(ctx, event, data, env),
            Self::Split(split) => split.event(ctx, event, data, env),
        }
    }

    fn lifecycle(&mut self, ctx: &mut LifeCycleCtx, event: &LifeCycle, data: &T, env: &Env) {
        match self {
            Self::Box(widget) => widget.lifecycle(ctx, event, data, env),
            Self::Split(split) => split.lifecycle(ctx, event, data, env),
        }
    }

    fn update(&mut self, ctx: &mut UpdateCtx, old_data: &T, data: &T, env: &Env) {
        match self {
            Self::Box(widget) => widget.update(ctx, old_data, data, env),
            Self::Split(split) => split.update(ctx, old_data, data, env),
        }
    }

    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &T, env: &Env) -> Size {
        match self {
            Self::Box(widget) => widget.layout(ctx, bc, data, env),
            Self::Split(split) => split.layout(ctx, bc, data, env),
        }
    }

    fn paint(&mut self, ctx: &mut PaintCtx, data: &T, env: &Env) {
        match self {
            Self::Box(widget) => widget.paint(ctx, data, env),
            Self::Split(split) => split.paint(ctx, data, env),
        }
    }
}
...
pub struct Split<T> {
    ...
    click_offset: f64,
    pub child1: WidgetPod<T, Box<dyn Widget<T>>>,
    pub child2: WidgetPod<T, SplitOrBox<T>>,

    pub child_info: Option<(ChildInfo, ChildInfo)>,
}
...
impl<T: Data> Split<T> {
    ...
    pub fn columns(child1: impl Widget<T> + 'static, child2: SplitOrBox<T>) -> Self {
    ...
    pub fn rows(child1: impl Widget<T> + 'static, child2: SplitOrBox<T>) -> Self {
